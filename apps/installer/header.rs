...
pub fn parse_lump_dir(data: &[u8], dir_off: u32, count: u32) -> ([LumpEntry; MAX_LUMPS], usize) {
    let empty = LumpEntry {
        type_tag: [0; 4],
        offset: 0,
        size: 0,
    ...
    for i in 0..n {
        ...
        if base + LUMP_ENTRY_SIZE > data.len() {
            return (lumps, i);
        }
...
pub fn find_lump(lumps: &[LumpEntry], count: usize, tag: &[u8; 4]) -> Option<(u32, u32)> {
    for i in 0..count {
        if lumps[i].is_type(tag) {
            return Some((lumps[i].offset, lumps[i].size));
        }
    }
    None
}
