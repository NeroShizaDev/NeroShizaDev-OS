...
impl Archive {
    ...
    pub fn add(&mut self, name: &[u8], data: &[u8]) -> bool {
        ...
        if name.len() > MAX_NAME   { return false; }
    ...
    pub fn add_jkl_block(&mut self, name: &[u8], jkl: &[u8]) -> bool {
        if jkl.len() < JKL_HEADER_SIZE { return false; }
        if jkl[0..4] != JKL_MAGIC { return false; }

        let algorithm = jkl[5];
        if algorithm > 2 { return false; }
        ...
        if JKL_HEADER_SIZE + encoded_size != jkl.len() { return false; }
        if original_size > u32::MAX as usize { return false; }
        if encoded_size > MAX_DATA { return false; }

        if !self.add(name, &jkl[JKL_HEADER_SIZE..]) { return false; }
...
pub fn pack(archive: &Archive, out: &mut [u8]) -> usize {
    ...
    if needed > out.len() { return 0; }

    let mut pos = 0usize;

    // Magic
    if pos + 5 > out.len() { return 0; }
...
pub fn unpack(data: &[u8]) -> Result<Archive, &'static str> {
    if data.len() < 7 { return Err("too short"); }

    // Magic
    if &data[0..5] != ARCH_MAGIC { return Err("bad magic"); }
    let count = data[5] as usize | ((data[6] as usize) << 8);
    if count > MAX_BLOCKS { return Err("too many blocks"); }

    let mut arch = Archive::new();
    let mut pos = 7usize;

    for _ in 0..count {
        if pos >= data.len() { return Err("truncated"); }

        // name_len
        let name_len = data[pos] as usize;
        pos += 1;
        if name_len > MAX_NAME { return Err("name too long"); }
        ...
        // algorithm
        if pos >= data.len() { return Err("truncated alg"); }
        b.algorithm = data[pos];
        if b.algorithm > 2 { return Err("unknown algorithm"); }
        pos += 1;

        // original_size
        if pos + 4 > data.len() { return Err("truncated orig_sz"); }
        ...
        // encoded_size
        if pos + 4 > data.len() { return Err("truncated enc_sz"); }
        ...
        if enc_sz > MAX_DATA { return Err("block data too large"); }
        if pos + enc_sz > data.len() { return Err("truncated data"); }
        b.data[..enc_sz].copy_from_slice(&data[pos..pos + enc_sz]);
        b.data_len = enc_sz;
        pos += enc_sz;

        if arch.count >= MAX_BLOCKS { return Err("too many blocks"); }
...
pub fn unpack_into(data: &[u8], out: &mut Archive) -> Result<(), &'static str> {
    ...
    for _ in 0..count {
        ...
        pos += 1;
        if pos + 4 > data.len() { return Err("truncated orig_sz"); }
        b.original_size = (data[pos] as u32) | ((data[pos+1] as u32) << 8)
            | ((data[pos+2] as u32) << 16) | ((data[pos+3] as u32) << 24);
        pos += 4;
        if pos + 4 > data.len() { return Err("truncated enc_sz"); }
        let enc_sz = ((data[pos] as u32) | ((data[pos+1] as u32) << 8)
            | ((data[pos+2] as u32) << 16) | ((data[pos+3] as u32) << 24)) as usize;
        pos += 4;
        if enc_sz > MAX_DATA { return Err("block data too large"); }
        if pos + enc_sz > data.len() { return Err("truncated data"); }
        b.data[..enc_sz].copy_from_slice(&data[pos..pos + enc_sz]);
