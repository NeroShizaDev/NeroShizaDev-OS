...
/// # Safety
/// Восстанавливает Mode 3h (80×25 текст) через прямую запись VGA-регистров.
/// После возврата 0xB8000 снова является текстовым буфером.
/// Вызывать только когда Mode 13h активен; вызов в неправильном состоянии
/// может оставить экран в артефактном режиме (без повреждения памяти).
pub unsafe fn restore_text_mode() {
    Port::<u8>::new(0x3C2).write(0x67);

    let seq: &[(u8, u8)] = &[
        (0x00, 0x03),
        (0x01, 0x00), // Dot Clock Divide
        (0x02, 0x03), // Write planes 0+1
        (0x03, 0x00),
        (0x04, 0x02), // Chain-4 off
    ];
    for &(i, d) in seq { vga_hw::write_seq(i, d); }

    vga_hw::write_reg(0x3D4, 0x3D5, 0x11, 0x0E); // unlock CRTC 0-7
    let crtc: &[(u8, u8)] = &[
        (0x00, 0x5F), (0x01, 0x4F), (0x02, 0x50), (0x03, 0x82),
        (0x04, 0x54), (0x05, 0x80), (0x06, 0xBF), (0x07, 0x1F),
        (0x08, 0x00), (0x09, 0x4F), // max scan line = 15 (16px font)
        (0x0A, 0x0D), (0x0B, 0x0E),
    ...
    let gc: &[(u8, u8)] = &[
        ...
        (0x04, 0x00), (0x05, 0x10), // text mode
        (0x06, 0x0E), // text mode memory map
        (0x07, 0x00), (0x08, 0xFF),
    ];
    for &(i, d) in gc { vga_hw::write_gc(i, d); }

    let _ = Port::<u8>::new(0x3DA).read(); // reset AC flip-flop
    let attr: &[(u8, u8)] = &[
        (0x00, 0x00), (0x01, 0x01), (0x02, 0x02), (0x03, 0x03),
        (0x04, 0x04), (0x05, 0x05), (0x06, 0x14), (0x07, 0x07),
        (0x08, 0x38), (0x09, 0x39), (0x0A, 0x3A), (0x0B, 0x3B),
        (0x0C, 0x3C), (0x0D, 0x3D), (0x0E, 0x3E), (0x0F, 0x3F),
        (0x10, 0x0C), // Mode Control (text)
        (0x11, 0x00), (0x12, 0x0F), (0x13, 0x08), (0x14, 0x00),
    ];
    for &(i, d) in attr { vga_hw::write_ac(i, d); }
    Port::<u8>::new(0x3C0).write(0x20);
}
