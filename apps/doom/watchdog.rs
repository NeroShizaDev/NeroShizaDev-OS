...
pub fn kernel_bite(code: BiteCode) -> ! {
    ...
    unsafe { super::vga_graphics::restore_text_mode(); }
    ...
    unsafe { fire_buf.copy_from_slice(&*core::ptr::addr_of!(BITE_FIRE_MODE)); }
    ...
    { let mut buf = [0u8; 20]; let l = fmt_u64_dec(fc, &mut buf); vga_write(8, 11, &buf[..l], ATTR_WARN); }

    // Таймаут: "ELAPSED: N > N"
    let (elapsed, timeout) = unsafe { (BITE_ELAPSED, BITE_TIMEOUT) };
    vga_write(9, 3, b"ELAPSED: ", ATTR_BG);
    { let mut buf = [0u8; 20]; let l = fmt_u64_dec(elapsed, &mut buf); vga_write(9, 12, &buf[..l], ATTR_WARN); }
    vga_write(9, 33, b" > ", ATTR_BG);
    { let mut buf = [0u8; 20]; let l = fmt_u64_dec(timeout, &mut buf); vga_write(9, 36, &buf[..l], ATTR_WARN); }
