...
/// Обязательно вызывать exit_font_mode() перед возвратом к обычному текстовому выводу.
...
/// После выхода VGA возвращается к обычной текстовой адресации.
...
pub fn detect_and_switch() -> VgaTextMode {
    ...
    // 90×30 сейчас намеренно отключён: QEMU принимает CRTC-записи
    // (readback проходит), но реально не меняет геометрию — экран
    // остаётся 80×25, а вывод 90 символов в строку делает текст нечитаемым.
    // Расширенный режим должен включаться только после надёжной верификации.
...
///
/// # Safety
/// Перепрограммирует ВСЕ VGA-регистры. После вызова дисплей отображает
/// обычный текстовый режим, а plane 2 содержит базовый ASCII-шрифт.
...
// Шаг 4: завершаем возврат в текстовый режим без прямой очистки текстовой памяти.
//
...
/// 4. Завершает возврат в стабильный текстовый режим без прямой очистки буфера
///
/// # Safety
/// Перепрограммирует все VGA-регистры и plane-2 шрифт.
/// Вызывать только когда Mode 13h (или другой граф. режим) активен.
/// После вызова текстовый режим снова активен.
pub unsafe fn restore_text_mode() {
    ...
    //   - Шаг 4: завершение возврата без прямой записи в текстовый буфер
