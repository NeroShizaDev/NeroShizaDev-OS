    ...
    for y in (top + 1..SCREEN_H).rev() {
        for x in 0..SCREEN_W {
            ...
            if pixel == 0 {
                ...
                let rand = fast_rand() % 4;
                let dst_x = (x + SCREEN_W + 1 - (rand as usize & 1)) % SCREEN_W;
...
#[inline(always)]
fn fclamp(v: f32) -> f32 {
    if v < 0.0 { 0.0 } else if v > MAX_HEAT { MAX_HEAT } else { v }
}
...
unsafe fn seed_fpu(mode: FireMode) {
    ...
    let top = SCREEN_H - FIRE_MAX_HEIGHT;
    match mode {
        FireMode::FpuNoise | FireMode::Classic => {
    ...
    for y in (top..SCREEN_H).rev() {
        ...
        for x in 0..SCREEN_W {
            let is_seed = match mode {
                FireMode::FpuNoise | FireMode::Classic => y == SCREEN_H - 1,
                FireMode::Dual => y == 0 || y == SCREEN_H - 1,
                FireMode::Inferno => {
                    y == 0 || y == SCREEN_H - 1 || x == 0 || x == SCREEN_W - 1
                }
            };
            if is_seed { continue; }
            let c = temp_row[x];
        ...
        if FPU_ENABLED {
            for y in top..SCREEN_H {
                for x in 0..SCREEN_W {
                    ...
                    let level = ((heat * (FIRE_LEVELS as f32 - 1.0)) as usize)
